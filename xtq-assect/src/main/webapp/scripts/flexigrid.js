...
(function($){
	...
	$.addFlex = function(t,p)
	{
		...
		p = $.extend({
			 ...
			 rpOptions: [10,15,20,25,40,50],
			 title: false,
			 pagestat: '显示从{from}到{to}, 共{total}条',
			 pagetext: '第',
			 outof: '页/共',
			 endof: '页',
			 findtext: '搜索',
		...
		var g = {
			...
			addData: function (data) { //parse data
				...
				//修改一个分页一个不分页grid的bug 从以下开始 包围了一个if(p.usepager) 条件 --zhangqingjing 20150805
				//以下是源代码
				if(p.usepager){
					if (p.dataType=='xml')
				...
				//修改一个分页一个不分页grid的bug 从这里结束  --zhangqingjing 20150805
					 ...
					 function(i,row)
					 	{
							...
							if (p.checkbox) {
								var cth = $('<th/>');
								var cthch = $('<input style="align: center" type="checkbox" value="' + $(tr).attr('id') +'"/>');
								var objTr = $(tr);
			...
			populate: function () { //get latest data
				...
				//修改多个grid 分页bug 从以下开始   20150805  --章青景
				//以下是源代码
				/*var param = [
				...
				//修改多个grid 分页bug 从这里结束   20150805  --章青景
				if (p.params)
					{
		...
		$(g.bDiv).after(g.pDiv);
		var html = ' <div class="pGroup"> <div id="pFirstButton"class="pFirst pButton"><span></span></div><div class="pPrev pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pcontrol">'+p.pagetext+' <input type="text" size="4" value="1" /> '+p.outof+'<span>1</span>'+p.endof+'</span></div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pNext pButton"><span></span></div><div class="pLast pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pReload pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pPageStat"></span></div>';
