...
App.QueuesRoute = Ember.Route.extend({
  ...
  beforeModel:function (transition) {
    ...
    controller.set('model', {message:'集群检查'});
  ...
  model: function() {
    ...
    return new Ember.RSVP.Promise(function (resolve,reject) {
      loadingController.set('model', {message:'通过检查'});
      store.checkOperator().then(function   (isOperator) {
        controller.set('isOperator', isOperator);

        loadingController.set('model', {message:'加载节点标签'});
        ...
        loadingController.set('model', {message:'加载队列'});
